...
# Build a balanced targets dataset with members from the published subset and non-members from the holdout subset.
def build_targets_df(
    ...
    *,
    ...
    publish_missing = [qi for qi in known_qids if qi not in publish_df.columns]
    holdout_missing = [qi for qi in known_qids if qi not in holdout_df.columns]
    if publish_missing or holdout_missing:
        raise ValueError(
            f"Missing known QI columns. publish_missing={publish_missing}, holdout_missing={holdout_missing}"
        )

    if target_id_col not in publish_df.columns or target_id_col not in holdout_df.columns:
    ...
    if targets_per_class > len(publish_df):
        raise ValueError(
            f"targets_per_class ({targets_per_class}) is larger than the published subset size ({len(publish_df)})."
        )
    if targets_per_class > len(holdout_df):
        raise ValueError(
            f"targets_per_class ({targets_per_class}) is larger than the holdout subset size ({len(holdout_df)})."
        )

    member_df = publish_df.sample(n=targets_per_class, random_state=seed, replace=False).copy()
    non_member_df = holdout_df.sample(n=targets_per_class, random_state=seed, replace=False).copy()
...
def main() -> None:
    ...
    parser.add_argument("--publish-size", type=int, default=None, help="Number of rows kept in the published subset.")
    parser.add_argument("--publish-frac", type=float, default=None, help="Fraction of rows kept in the published subset.")
